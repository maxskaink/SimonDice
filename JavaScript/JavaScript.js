...
const SCREEN_LEVEL = document.getElementById("ScreenLevel");
...
class Juego {
    ...
    mostrarLevel() {
        if (this.GANAR){
            SCREEN_LEVEL.innerHTML = `Comienze a jugar :)`;
        }else {
            SCREEN_LEVEL.innerHTML = `Esta en el nivel ${this.nivel}`;
        }
    }
    siEmpezar() {
    ...
    perdioElJuego() {
        swal("Lo siento...:c", "Has perdido!", "error")
            .then(() => {
                this.eliminarEventoClick();
                this.inicializar();
            })
